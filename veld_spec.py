...
def validate(obj_to_validate, node: Node, path=""):
    ...
    def handle_node_disjunction(obj_to_validate, node: NodeDisjunction, path):
        ...
        if not is_one_valid:
            ...
            return (False, f"all possilbe options are invalid: ({all_errors})")
        else:
            return (True, None)
    ...
    def handle_node_dict(obj_to_validate, node: NodeDict, path):
        if type(obj_to_validate) is not dict:
            ...
            for node_mapping in node.content:
                if node_mapping.content.content not in obj_to_validate and not node_mapping.is_optional:
                    return (False, f"non-optional key missing: '{node_mapping.content.content}', at: {path}/")
                else:
                    node_key = node_mapping.content.content
                    obj_value = obj_to_validate.pop(node_key, None)
                    path_sub = path + "/" + node_key
                    if obj_value is not None:
                        result = validate(obj_value, node_mapping.target, path_sub)
                        if not result[0]:
                            return result
                    elif not node_mapping.target.is_optional:
